...
from models.GGEConvNet import GGEConvNet_Small
from models.GGEConvNet import GGEConvNet_Large
...
class Train_Knowledge_Distillation:
    ...
    def save_model(self, model, optimizer, val_accuracy, epoch):
        save_path = f"{self.args.output_dir}/Fold{self.args.fold}/akd_{type(model).__name__.lower()}/epoch{epoch}_akd_{type(model).__name__.lower()}_{val_accuracy:.4f}.pth"
